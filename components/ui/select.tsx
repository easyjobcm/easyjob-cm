...
export function Select({ value, defaultValue = '', onValueChange, children }: SelectProps) {
  ...
  return <SelectContext.Provider value={{ value: resolvedValue, setValue }}>{children}</SelectContext.Provider>
}
...
export function SelectContent({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('mt-2 space-y-1 rounded-xl border border-border bg-card p-1', className)} {...props} />
}
