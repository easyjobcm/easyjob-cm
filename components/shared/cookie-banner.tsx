...
import { Cookie, Shield, Sparkles, BarChart3, Megaphone, Settings2, Lock } from 'lucide-react'
...
function setConsentCookie(consent: ConsentState) {
  const value = encodeURIComponent(JSON.stringify({ v: consent.version, a: consent.analytics, m: consent.marketing, p: consent.preferences }))
  const expires = new Date()
  expires.setFullYear(expires.getFullYear() + 1)
  document.cookie = `${CONSENT_COOKIE}=${value}; expires=${expires.toUTCString()}; path=/; SameSite=Lax`
}

function persistConsent(opts: { analytics: boolean; marketing: boolean; preferences: boolean }): ConsentState {
...
function ToggleRow({ label, desc, value, onChange, Icon, color }: ToggleRowProps) {
  return (
    <motion.div
      whileHover={{ y: -1 }}
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11.5px] text-[#6B7280] mt-0.5 leading-relaxed">{desc}</p>
        </div>
        <motion.button
          whileTap={{ scale: 0.92 }}
...
export function CookieBanner() {
  ...
  const cardVariants = {
    ...
    visible: { y: 0,  opacity: 1, scale: 1, transition: { type: 'spring' as const, damping: 24, stiffness: 280 } },
    exit:    { y: 80, opacity: 0, scale: 0.96, transition: { duration: 0.22, ease: 'easeIn' as const } },
  }

  const sheetVariants = {
    hidden:  { y: '100%' },
    visible: { y: 0, transition: { type: 'spring' as const, damping: 28, stiffness: 320 } },
      ...
      {visible && !showDetails && (
        ...
        >
          ...
          <div
            ...
            style={{ background: 'radial-gradient(ellipse at 30% 50%, #7C3AED 0%, transparent 60%)' }}
          />

          {/* Card */}
          <div
            className="relative bg-white rounded-3xl overflow-hidden border border-[#E5E7EB]"
            style={{ boxShadow: '0 24px 64px rgba(91,33,182,0.22), 0 8px 24px rgba(0,0,0,0.08)' }}
          >
                ...
                <div className="relative shrink-0">
                  <motion.div
                    ...
                    style={{
                      ...
                      boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.8), 0 4px 12px rgba(124,58,237,0.18)',
                    }}
                  >
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <motion.button
                  ...
                  style={{
                    ...
                    boxShadow: '0 4px 16px rgba(124,58,237,0.4), 0 2px 4px rgba(91,33,182,0.2)',
                  }}
                >
                  {t('cookie.accept')}
                </motion.button>
            ...
            <div className="px-5 pt-5 pb-4 max-w-2xl mx-auto max-h-[78vh] overflow-y-auto">
              <div className="flex items-start justify-between mb-5">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-[16px] font-bold text-[#111827] leading-tight">{t('cookie.title')}</h3>
                    <p className="text-[11px] text-[#9CA3AF] mt-0.5">{t('cookie.subtitle')}</p>
                  </div>
                </div>
                <motion.button
                  whileTap={{ scale: 0.97 }}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-1.5">
                      <p className="text-[13px] font-semibold text-[#111827]">{t('cookie.necessary')}</p>
                      <span className="text-[9.5px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-[#7C3AED] text-white">
                        {t('cookie.alwaysOn')}
                      </span>
                    </div>
                    <p className="text-[11.5px] text-[#6B7280] mt-1">{t('cookie.necessaryDesc')}</p>
                  </div>
              ...
              <ToggleRow
                ...
                Icon={({ className }) => <BarChart3 className={className} style={{ color: analytics ? '#2563EB' : '#9CA3AF' }} />}
                color="#2563EB"
              />
              <ToggleRow
                ...
                Icon={({ className }) => <Megaphone className={className} style={{ color: marketing ? '#DC2626' : '#9CA3AF' }} />}
                color="#DC2626"
              />
              <ToggleRow
                ...
                Icon={({ className }) => <Settings2 className={className} style={{ color: preferences ? '#059669' : '#9CA3AF' }} />}
                color="#059669"
              />
