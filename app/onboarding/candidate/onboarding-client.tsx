...
const STEPS = [
  { id: 1, title: 'Informations personnelles', titleEn: 'Personal Info', icon: User },
  { id: 2, title: 'Localisation', titleEn: 'Location', icon: MapPin },
  { id: 3, title: 'Competences', titleEn: 'Skills', icon: Briefcase },
  { id: 4, title: 'Paiement Mobile', titleEn: 'Mobile Payment', icon: CreditCard },
]

const CAMEROON_CITIES = [
  'Douala', 'Yaounde', 'Bafoussam', 'Bamenda', 'Garoua', 
  'Maroua', 'Ngaoundere', 'Bertoua', 'Limbe', 'Kribi',
  'Buea', 'Ebolowa', 'Nkongsamba', 'Edea'
]

const COMMON_SKILLS = [
  'Service client', 'Vente', 'Restauration', 'Caisse', 'Manutention',
  'Conduite moto', 'Conduite voiture', 'Securite', 'Nettoyage', 'Cuisine',
  'Hotesse accueil', 'Evenementiel', 'Informatique', 'Anglais', 'Francais'
]

export function OnboardingClient({ user, profile, categories }: OnboardingClientProps) {
  const router = useRouter()
  const { locale } = useI18n()
  const supabase = createClient()

  const [currentStep, setCurrentStep] = React.useState(profile?.onboarding_step || 1)
  ...
  const handleNext = () => {
    ...
    if (currentStep === 2) {
      if (!formData.city) {
        setError(locale === 'fr' ? 'Veuillez selectionner une ville' : 'Please select a city')
        return
      }
    }
    if (currentStep === 3) {
      if (formData.skills.length < 2) {
        setError(locale === 'fr' ? 'Selectionnez au moins 2 competences' : 'Select at least 2 skills')
        return
      }
    }
    if (currentStep === 4) {
      if (!formData.momo_provider || !formData.momo_number) {
        setError(locale === 'fr' ? 'Veuillez remplir vos informations Mobile Money' : 'Please fill your Mobile Money info')
        return
      }
    }
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder={locale === 'fr' ? 'Votre prenom' : 'Your first name'}
              />
            </div>
          ...
          <div className="space-y-4">
            ...
            {formData.skills.length > 0 && (
              <p className="text-sm text-primary">
                {formData.skills.length} {locale === 'fr' ? 'competence(s) selectionnee(s)' : 'skill(s) selected'}
              </p>
            )}
          </div>
        )
      ...
      <div className="bg-card border-b border-border px-4 py-6">
        ...
        <div className="flex items-center gap-3 mt-4">
          ...
          <span className="font-medium text-foreground">
            {locale === 'fr' ? STEPS[currentStep - 1].title : STEPS[currentStep - 1].titleEn}
          </span>
        </div>
      </div>
