...
export default function ContractSignPage() {
  ...
  const [userRole, setUserRole] = useState<"candidate" | "company" | null>(
    null,
  );
  ...
  const isFullySigned =
    contract.status === "completed" || contract.status === "fully_signed";
  const canSign =
    userRole === "candidate"
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>
                ...
                {contract.job?.end_date &&
                  contract.job.end_date !== contract.job.start_date &&
            ...
            <div className="flex items-center gap-2">
              ...
              <span>
                {formatTime(contract.job?.start_time)} -{" "}
                {formatTime(contract.job?.end_time)}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <MapPin className="h-4 w-4 text-muted-foreground" />
              <span>
                {contract.job?.address}, {contract.job?.city}
              </span>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="prose prose-sm max-w-none text-muted-foreground">
              <p>
                Le present contrat de mission est conclu entre les parties
                susmentionnees pour l&apos;execution de la mission decrite
                ci-dessus.
              </p>
              <h4 className="text-foreground font-medium mt-4">
                Obligations du Candidat
              </h4>
              <ul className="list-disc pl-4 space-y-1">
                <li>
                  Se presenter a l&apos;heure convenue sur le lieu de travail
                </li>
              ...
              <p>
                Le candidat sera remunere au taux horaire indique ci-dessus. Le
                paiement sera effectue via Mobile Money dans les 48 heures
              ...
              {contract.candidate_signed_at ? (
                <div className="flex items-center gap-2 text-green-600">
                  ...
                  <span className="text-sm">
                    Signe le {formatDate(contract.candidate_signed_at)}
                  </span>
                </div>
              ) : (
            ...
            <div className="flex items-center justify-between">
              ...
              {contract.company_signed_at ? (
                <div className="flex items-center gap-2 text-green-600">
                  <CheckCircle className="h-4 w-4" />
                  <span className="text-sm">
                    Signe le {formatDate(contract.company_signed_at)}
                  </span>
                </div>
              ) : (
                <Badge variant="outline">En attente</Badge>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-start gap-2">
                ...
                >
                  ...
                  comprends que cette signature electronique a la meme valeur
                  legale qu&apos;une signature manuscrite.
                </label>
              </div>
