...
export default function MissionDetailPage() {
  ...
  const calculateProgress = () => {
    ...
    if (mission.status !== 'in_progress' || !mission.actual_start_time) return 0

    const start = new Date(mission.actual_start_time).getTime()
    const scheduledEnd = new Date(`${mission.scheduled_date}T${mission.scheduled_end_time}`).getTime()
    const now = Date.now()
  ...
  const isToday = new Date(mission.scheduled_date).toDateString() === new Date().toDateString()
  const canCheckIn = isToday && ['pending', 'confirmed'].includes(mission.status)
      ...
      <div className="bg-gradient-to-br from-violet-600 to-violet-800 text-white p-4 rounded-b-3xl">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-white/80">{mission.job?.company?.company_name}</p>
          </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              {isToday && <Badge className="bg-green-100 text-green-700 text-xs">Aujourd&apos;hui</Badge>}
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{mission.job?.address}, {mission.job?.city}</span>
            </div>
              ...
              {mission.actual_start_time ? (
                <div className="flex items-center gap-2 text-green-600">
                  ...
                  <span className="text-sm">
                    {new Date(mission.actual_start_time).toLocaleTimeString('fr-FR', {
                      hour: '2-digit',
                      minute: '2-digit'
                    })}
                  </span>
            ...
            <div className="flex items-center justify-between">
              ...
              {mission.actual_end_time ? (
                <div className="flex items-center gap-2 text-green-600">
                  <CheckCircle className="h-4 w-4" />
                  <span className="text-sm">
                    {new Date(mission.actual_end_time).toLocaleTimeString('fr-FR', {
                      hour: '2-digit',
                      minute: '2-digit'
                    })}
                  </span>
      ...
      <Dialog open={showRatingDialog} onOpenChange={setShowRatingDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Comment s&apos;est passee votre experience avec {mission.job?.company?.company_name} ?
            </DialogDescription>
          </DialogHeader>
