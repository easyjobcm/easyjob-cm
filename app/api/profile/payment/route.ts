 ...
 * / `momo_verified_*` / `momo_reject_reason` lui-même : la migration T6 a
 * protégé ces 5 colonnes par un trigger `BEFORE UPDATE` (le flag session
 * `easyjob.system_update` n'étant pas posé pour un client). On passe donc
 * par le RPC SECURITY DEFINER `candidate_update_momo` qui réinitialise
 * proprement TOUT le cycle de vérification.
 *
