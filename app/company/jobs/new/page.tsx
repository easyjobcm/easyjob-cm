...
const jobTypes = [
  { id: 'shift', labelFr: 'Mission ponctuelle', labelEn: 'One-time shift', descFr: 'Pour un jour specifique', descEn: 'For a specific day' },
  { id: 'contract', labelFr: 'Contrat temporaire', labelEn: 'Temporary contract', descFr: 'Plusieurs jours/semaines', descEn: 'Multiple days/weeks' },
]
...
export default function NewJobPage() {
  ...
  const commonRequirements = [
    { id: 'exp', labelFr: 'Experience recommandee', labelEn: 'Experience recommended' },
  ...
  const commonBenefits = [
    ...
    { id: 'regular', labelFr: 'Missions regulieres possibles', labelEn: 'Regular missions possible' },
  ]

  const toggleItem = (array: string[], item: string, field: 'requirements' | 'benefits') => {
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <div className="px-6 py-4 border-b border-border">
        ...
        <p className="text-xs text-muted-foreground mt-2">
          {locale === 'fr' ? steps[currentStepIndex].labelFr : steps[currentStepIndex].labelEn}
          {' • '}
      ...
      <div className="flex-1 px-6 py-6 overflow-y-auto">
          ...
          <div className="space-y-6">
            <div>
              ...
              <Input
                ...
                placeholder={locale === 'fr' ? 'Ex: Serveur/Serveuse' : 'Ex: Waiter/Waitress'}
                className="mt-1.5"
              />
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {jobTypes.map((type) => (
                  <button
                    key={type.id}
                    onClick={() => setFormData(prev => ({ ...prev, type: type.id as 'shift' | 'contract' }))}
                    className={cn(
        ...
        {currentStep === 'schedule' && (
          <div className="space-y-6">
            ...
            <div>
              <label className="text-sm font-medium text-foreground">
                {locale === 'fr' ? 'Nombre de personnes recherchees' : 'Number of workers needed'}
              </label>
              <div className="flex items-center gap-3 mt-1.5">
                ...
                <span className="text-xl font-bold w-12 text-center">{formData.workersNeeded}</span>
                <Button
                  variant="outline"
                  size="icon"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {commonRequirements.map((req) => (
                  <button
                    key={req.id}
                    onClick={() => toggleItem(formData.requirements, req.id, 'requirements')}
                    className={cn(
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {commonBenefits.map((benefit) => (
                  <button
                    key={benefit.id}
                    onClick={() => toggleItem(formData.benefits, benefit.id, 'benefits')}
                    className={cn(
                      'px-3 py-2 rounded-full text-sm font-medium transition-all',
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {locale === 'fr' ? 'Minimum recommande: 1000 XAF/h' : 'Recommended minimum: 1000 XAF/h'}
              </p>
            </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  <AlertTriangle className={cn(
                    'h-5 w-5',
                    formData.urgent ? 'text-warning' : 'text-muted-foreground'
                  )} />
                  <div className="flex-1">
                    <p className="font-medium text-foreground">
                      {locale === 'fr' ? 'Marquer comme urgent' : 'Mark as urgent'}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {locale === 'fr'
                  ...
                  <div className={cn(
                    ...
                    {formData.urgent && <Check className="h-4 w-4 text-warning-foreground" />}
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span>{jobCategories.find(c => c.id === formData.category)?.[locale === 'fr' ? 'labelFr' : 'labelEn']}</span>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {formData.requirements.map(req => (
                          <span key={req} className="text-xs bg-primary/10 text-primary px-2 py-1 rounded-full">
                            {commonRequirements.find(r => r.id === req)?.[locale === 'fr' ? 'labelFr' : 'labelEn']}
                          </span>
                        ))}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {formData.benefits.map(benefit => (
                          <span key={benefit} className="text-xs bg-accent/10 text-accent px-2 py-1 rounded-full">
                            {commonBenefits.find(b => b.id === benefit)?.[locale === 'fr' ? 'labelFr' : 'labelEn']}
                          </span>
                        ))}
                      </div>
                    </div>
        ...
        {currentStep === 'review' ? (
          ...
          >
            {loading ? (
              ...
              locale === 'fr' ? 'Publier l\'offre' : 'Publish job'
            )}
          </Button>
        ) : (
          <Button
