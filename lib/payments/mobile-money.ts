...
class MTNMoMoService {
  ...
  async initiatePayment(request: PaymentRequest): Promise<PaymentResponse> {
    ...
    if (process.env.MOBILE_MONEY_SANDBOX === 'true') {
      ...
      return {
        success,
        transactionId: success ? `mtn_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` : undefined,
        status: success ? 'pending' : 'failed',
        message: success ? 'Paiement initie avec succes' : 'Echec du paiement (sandbox simulation)',
  ...
  async checkTransactionStatus(transactionId: string): Promise<TransactionStatus> {
    ...
    if (process.env.MOBILE_MONEY_SANDBOX === 'true') {
      ...
      const statuses: TransactionStatus['status'][] = ['completed', 'completed', 'pending', 'failed']
      const status = statuses[Math.floor(Math.random() * statuses.length)]
      
      return {
        transactionId,
        status,
        payerMessage: status === 'completed' ? 'Transaction reussie' : 'Transaction en cours'
      }
    }
...
class OrangeMoneyService {
  ...
  async initiatePayment(request: PaymentRequest): Promise<PaymentResponse> {
    ...
    if (process.env.MOBILE_MONEY_SANDBOX === 'true') {
      ...
      return {
        success,
        transactionId: success ? `om_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` : undefined,
        status: success ? 'pending' : 'failed',
        message: success ? 'Paiement initie. Composez *144# pour confirmer.' : 'Echec du paiement',
        externalReference: request.reference
      }
    }
  ...
  async checkTransactionStatus(transactionId: string): Promise<TransactionStatus> {
    if (process.env.MOBILE_MONEY_SANDBOX === 'true') {
      const statuses: TransactionStatus['status'][] = ['completed', 'completed', 'pending']
      return {
        transactionId,
