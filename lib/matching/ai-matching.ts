...
export async function calculateMatchScore(
  ...
  if (!candidate || !job) {
    return {
      score: 0,
      breakdown: { skills: 0, location: 0, availability: 0, experience: 0, reliability: 0 },
  ...
  if (job.required_skills && job.required_skills.length > 0 && candidate.skills) {
    const candidateSkillNames = candidate.skills.map((s: { skill_name: string }) => 
    ...
    breakdown.skills = (matchedSkills.length / job.required_skills.length) * 100
  ...
  if (job.start_date && candidate.availability && candidate.availability.length > 0) {
...
export async function updateReliabilityScore(candidateId: string): Promise<void> {
  const supabase = await createClient()
  ...
  if (candidate.total_missions > 0) {
    const completionRate = candidate.completed_missions / candidate.total_missions
