...
function run(command: string, args: string[], captureOutput = false): string {
  ...
  if (result.status !== 0) {
    if (captureOutput) {
      const errorOutput = (result.stderr || result.stdout || '').toString().trim()
      if (errorOutput) {
...
function checkDockerRunning() {
  ...
  if (result.error || result.status !== 0) {
    ...
    console.error('Start Docker Desktop, wait until it is healthy, then rerun bootstrap.')
    process.exit(1)
  }
...
function ensureEnvFile() {
  ...
  console.error('Create .env.local (or add .env.local.example) before running bootstrap.')
  process.exit(1)
}
...
function writeSupabaseEnvFromStatus(statusEnv: Record<string, string>) {
  ...
  if (!apiUrl || !anonKey || !serviceRoleKey) {
    logError('Unable to extract API_URL, ANON_KEY, SERVICE_ROLE_KEY from `supabase status -o env`.')
    process.exit(1)
  }

  let envContent = readFileSync('.env.local', 'utf8')
  envContent = upsertEnvValue(envContent, 'NEXT_PUBLIC_SUPABASE_URL', apiUrl)
  envContent = upsertEnvValue(envContent, 'NEXT_PUBLIC_SUPABASE_ANON_KEY', anonKey)
  envContent = upsertEnvValue(envContent, 'NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY', anonKey)
  envContent = upsertEnvValue(envContent, 'SUPABASE_URL', apiUrl)
  envContent = upsertEnvValue(envContent, 'SUPABASE_SERVICE_ROLE_KEY', serviceRoleKey)
...
async function main() {
  ...
  run('npx', ['supabase', 'gen', 'types', 'typescript', '--local', '--output', 'src/types/database.ts'])
  ...
  const mailpitUrl = statusEnv.INBUCKET_URL ?? statusEnv.MAILPIT_URL ?? 'http://127.0.0.1:54324'
  ...
  console.log(`${COLORS.blue}→ Seed:${COLORS.reset} ${options.skipSeed ? 'skipped' : 'completed'}`)
  console.log('')
