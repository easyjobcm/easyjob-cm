...
async function getOrCreateAuthUser(phone: string, role: SeedUser["role"]) {
  ...
  if (!existing) {
    throw (
      createError ?? new Error(`Unable to create/find auth user for ${phone}`)
    );
  }
...
async function seedTestData() {
  ...
  const categoryMap = new Map(
    allCategories?.map((c) => [c.name_fr, c.id]) || [],
  );
  ...
  const jobs = [
    {
      ...
      description:
        "Nous recherchons des serveurs expérimentés pour une grande soirée VIP. Tenue correcte exigée.",
      job_type: "shift",
    ...
    },
    {
      ...
      description:
        "Nettoyage du restaurant tous les matins avant l'ouverture. Matériel fourni.",
      job_type: "contract",
      start_date: tomorrow.toISOString().split("T")[0],
    ...
    },
    {
      ...
      description:
        "Accueil des clients, gestion des réservations, service de conciergerie.",
      job_type: "shift",
      start_date: tomorrow.toISOString().split("T")[0],
      end_date: tomorrow.toISOString().split("T")[0],
      start_time: "22:00",
    ...
    },
    {
      ...
      description:
        "Accueil des participants, gestion des badges, orientation. Anglais et français requis.",
      job_type: "shift",
    ...
    },
    {
      ...
      description:
        "Déchargement et rangement des livraisons. Port de charges lourdes.",
      job_type: "shift",
      start_date: tomorrow.toISOString().split("T")[0],
      end_date: tomorrow.toISOString().split("T")[0],
      start_time: "08:00",
  ...
  const missions = [
    ...
    },
    {
      ...
      actual_end_time: new Date(
        yesterday.getTime() + 12 * 60 * 60 * 1000,
      ).toISOString(),
      status: "completed",
